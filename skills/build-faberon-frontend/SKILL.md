...
Enter `frontend` before running application, pnpm, Node, Vite, lint, test, or build commands. Never run package-manager commands from the repository root. Keep pnpm's store outside the repository; when a sandbox cannot use the normal user-level store, use `/tmp/faberon-pnpm-store`.

## Workflow

1. Put route declarations in `src/app` and page containers in `src/pages/<feature>`.
...
6. Add product visual values to the global Tailwind theme in `src/styles/index.css`.
